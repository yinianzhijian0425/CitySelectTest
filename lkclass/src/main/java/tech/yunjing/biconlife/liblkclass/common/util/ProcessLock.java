...
public final class ProcessLock implements Closeable {
     ...
     */
    private final static DoubleKeyValueMap<String, Integer, ProcessLock> LOCK_MAP = new DoubleKeyValueMap<>();

    static {
        File dir = LK.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE);
    ...
    private static ProcessLock tryLockInternal(final String lockName, final String hash, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(
                        LK.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE),
                        hash);
                ...
                LKLogUtil.d("tryLock: " + lockName + ", " + ignored.getMessage());
                IOUtil.closeQuietly(stream);

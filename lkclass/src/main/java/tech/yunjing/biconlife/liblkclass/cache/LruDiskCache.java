package tech.yunjing.biconlife.liblkclass.cache;
...
import tech.yunjing.biconlife.liblkclass.common.util.LKFileUtil;
import tech.yunjing.biconlife.liblkclass.common.util.LKLogUtil;
import tech.yunjing.biconlife.liblkclass.common.util.MD5;
import tech.yunjing.biconlife.liblkclass.common.util.ProcessLock;
import tech.yunjing.biconlife.liblkclass.db.sqlite.WhereBuilder;
import tech.yunjing.biconlife.liblkclass.global.config.DbConfigs;
import tech.yunjing.biconlife.liblkclass.global.ex.DbException;
import tech.yunjing.biconlife.liblkclass.global.ex.FileLockedException;
...
public final class LruDiskCache {
     ...
     * 限制最多100M文件
     */
    private static final long LIMIT_SIZE = 1024L * 1024L * 100L;
    /**
     * 3s
     */
    private static final int LOCK_WAIT = 1000 * 3;
    ...
    private LruDiskCache(String dirName) {
        this.cacheDb = LK.getDb(DbConfigs.HTTP.getConfig());
        this.cacheDir = LKFileUtil.getCacheDir(dirName);
    ...
    public LruDiskCache setMaxSize(long maxSize) {
        if (maxSize > 0L) {
            long diskFreeSize = LKFileUtil.getDiskAvailableSize();
            ...
            public void run() {
                if (available) {
                    ...
                    try {
                        ...
                        boolean judge = count > LIMIT_COUNT + 10;
                        if (judge) {
                    ...
                    // trim disk
                    try {
                        while (LKFileUtil.getFileOrDirSize(cacheDir) > diskCacheSize) {
                            List<DiskCacheEntity> rmList = cacheDb.selector(DiskCacheEntity.class)

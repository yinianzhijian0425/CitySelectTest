...
import tech.yunjing.biconlife.liblkclass.lkbase.ViewInjector;
import tech.yunjing.biconlife.liblkclass.lkbase.LK;
import tech.yunjing.biconlife.liblkclass.view.annotation.LKContentView;
import tech.yunjing.biconlife.liblkclass.view.annotation.LKEvent;
import tech.yunjing.biconlife.liblkclass.view.annotation.LKViewInject;
...
public final class ViewInjectorImpl implements ViewInjector {
    ...
    public static void registerInstance() {
        ...
        LK.Ext.setViewInjector(instance);
    }
    ...
    public void inject(Activity activity) {
        ...
        try {
            LKContentView LKContentView = findContentView(handlerType);
            if (LKContentView != null) {
                int viewId = LKContentView.value();
                if (viewId > 0) {
    ...
    private static LKContentView findContentView(Class<?> thisCls) {
        ...
        LKContentView LKContentView = thisCls.getAnnotation(LKContentView.class);
        if (LKContentView == null) {
            return findContentView(thisCls.getSuperclass());
        }
        return LKContentView;
    }
    ...
    private static void injectObject(Object handler, Class<?> handlerType, ViewFinder finder) {
        ...
        if (fields != null && fields.length > 0) {
            for (Field field : fields) {
                ...
                LKViewInject LKViewInject = field.getAnnotation(LKViewInject.class);
                if (LKViewInject != null) {
                    try {
                        View view = finder.findViewById(LKViewInject.value(), LKViewInject.parentId());
                        if (view != null) {
        ...
        if (methods != null && methods.length > 0) {
            for (Method method : methods) {
                ...
                //检查当前方法是否是event注解的方法
                LKEvent LKEvent = method.getAnnotation(LKEvent.class);
                if (LKEvent != null) {
                    try {
                        // id参数
                        int[] values = LKEvent.value();
                        int[] parentIds = LKEvent.parentId();
                        ...
                        for (int i = 0; i < values.length; i++) {
                            ...
                            if (value > 0) {
                                ...
                                EventListenerManager.addEventMethod(finder, info, LKEvent, handler, method);
                            }
                        }
                    } catch (Throwable ex) {
                        LKLogUtil.e(ex.getMessage(), ex);

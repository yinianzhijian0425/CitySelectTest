...
public class LKStrParserUtil {
    ...
    private static LKStrParserUtil LKStrParserUtil = new LKStrParserUtil();

    public static LKStrParserUtil getInstance() {
        return LKStrParserUtil;
    }
    ...
    private int getChsAscii(String chs) {
        ...
        try {
            ...
            boolean judge1 = bytes.length > 2 || bytes.length <= 0;
            if (judge1) {
                throw new RuntimeException("illegal resource string");
            }
            boolean judge2 = bytes.length == 1;
            if (judge2) {
                asc = bytes[0];
            }
            boolean judge3 = bytes.length == 2;
            if (judge3) {
            ...
            System.out.println("汉字转码错误:ChineseSpelling.class-getChsAscii(String chs)" + e);
        }
        return asc;
    }
    ...
    public String convert(String str) {
        ...
        boolean judge = ascii > 0 && ascii < 160;
        if (judge) {

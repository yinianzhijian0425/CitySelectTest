 ...
 */
public class BCImageDisplayActivity  extends AppBaseActivity {
    ...
    protected void initData() {
        super.initData();
        try {
            urls = (List<String>) getIntent().getExtras().getSerializable(UrlsLogo);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if(urls == null){
...
//                    View.SYSTEM_UI_FLAG_LAYOUT_STABLE//布局位于状态栏下方
...
//            ScaleAnimation scaleAnimation = new ScaleAnimation(1f, Float.intBitsToFloat(width), 1f, Float.intBitsToFloat(height));
            ...
            Animation animation = AnimationUtils.loadAnimation(this, R.anim.big_image_scale_shrink);
        ...
        rl_bcidd_background_layout.startAnimation(AnimationUtils.loadAnimation(this, tech.yunjing.biconlife.liblkclass.R.anim.anim_window_bottom_dialog_cancel_alpha));
    }

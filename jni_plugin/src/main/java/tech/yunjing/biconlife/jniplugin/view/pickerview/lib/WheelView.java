package tech.yunjing.biconlife.jniplugin.view.pickerview.lib;
...
import tech.yunjing.biconlife.jniplugin.view.pickerview.adapter.WheelAdapter;
import tech.yunjing.biconlife.jniplugin.view.pickerview.listener.OnItemSelectedListener;
import tech.yunjing.biconlife.jniplugin.view.pickerview.model.IPickerViewData;
...
public class WheelView extends View {
     ...
     * 原始数值为11，如出问题恢复成11即可，最少为5，只可设置奇数，不允许输入偶数；
     */
    int itemsVisible = 11;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            LKLogUtil.e("WheelView" + "出错了！adapter.getItemsCount() == 0，联动数据不匹配");
        }
        if (!isLoop) {//不循环的情况
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    if (!TextUtils.isEmpty(contentText) && contentText.contains("_")) {
                        canvas.drawText(contentText.split("_")[0], drawOutContentStart, maxTextHeight, paintOuterText);
                    } else {
                    ...
                    if (!TextUtils.isEmpty(contentText) && contentText.contains("_")) {
                        canvas.drawText(contentText.split("_")[0], drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintCenterText);
                    } else {
                    ...
                    if (!TextUtils.isEmpty(contentText) && contentText.contains("_")) {
                        canvas.drawText(contentText.split("_")[0], drawCenterContentStart, Y, paintCenterText);
                    } else {
     ...
     * 设置可见item数量
     * 绘制几个条目，实际上第一项和最后一项Y轴压缩成0%了，所以可见的数目实际为9
     * 原始数值为11，如出问题恢复成11即可，最少为3，只可设置奇数，不允许输入偶数；
     */

...
import tech.yunjing.biconlife.libbaselib.util.ThemeUtils;
...
public class MainNavigateTabBar extends LinearLayout implements View.OnClickListener {
    ...
    public MainNavigateTabBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mNormalTextColor = (tabTextColor != null ? tabTextColor : context.getResources().getColorStateList(R.color.color_A7A7A7));
    ...
    public void addTab(Class frameLayoutClass, TabParam tabParam) {
        int defaultLayout = R.layout.layout_main_bottom;
...
//        }
        ...
        holder.tabIcon = (ImageView) view.findViewById(R.id.iv_tab_icon);
        holder.tabTitle = ((TextView) view.findViewById(R.id.tv_tab_title));
       ...
       /* if(holder.tabIndex==0){
            holder.tv_msg.setVisibility(VISIBLE);
        }*/
        if (mTabTextSize != 0) {
    ...
    private void showFragment(ViewHolder holder) {
        ...
        LKBaseFragment fragment = (LKBaseFragment) mFragmentActivity.getSupportFragmentManager().findFragmentByTag(holder.tag);
        if (fragment == null) {
            ...
            for (ViewHolder holder1 : mViewHolderList) {
                if (holder.tag != holder1.tag) {
                    Fragment fragment1 = mFragmentActivity.getSupportFragmentManager().findFragmentByTag(holder1.tag);
    ...
    private LKBaseFragment getFragmentInstance(String tag) {
        LKBaseFragment fragment = null;
        for (ViewHolder holder : mViewHolderList) {
            if (TextUtils.equals(tag, holder.tag)) {
                try {
                    fragment = (LKBaseFragment) Class.forName(holder.fragmentClass.getName()).newInstance();
    ...
    public static class TabParam {
        public int backgroundColor = R.color.color_FAFAFA;
        //        public int backgroundColor = R.color.transparent_background;
    ...
    public void setTvmsg(int i) {
        ...
        holder.tv_msg.setText(i + "");
        if (0 == i) {
            holder.tv_msg.setVisibility(GONE);
        } else {
            holder.tv_msg.setVisibility(VISIBLE);
        }

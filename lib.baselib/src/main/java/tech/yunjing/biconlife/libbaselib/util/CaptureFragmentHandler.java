...
import tech.yunjing.biconlife.libbaselib.view.scan.decoding.ViewfinderResultPointCallback;
...
public class CaptureFragmentHandler extends Handler {
    ...
    private final DecodeMainThread decodeThread;
    private State state;
    ...
    public CaptureFragmentHandler(Activity activity, ScanFragment fragment, Vector<BarcodeFormat> decodeFormats,
                                  String characterSet) {
        this.fragment = fragment;
        this.activity = activity;
        decodeThread = new DecodeMainThread(fragment, decodeFormats, characterSet,
    ...
    public void handleMessage(Message message) {
        if (message.what == R.id.auto_focus) {//Log.d(TAG, "Got auto-focus message");
            ...
            LKLogUtil.e("Got restart preview message");
            restartPreviewAndDecode();

        } else if (message.what == R.id.decode_succeeded) {
            LKLogUtil.e("Got decode succeeded message");
            state = State.SUCCESS;
            Bundle bundle = message.getData();

            /***********************************************************************/
            Bitmap barcode = bundle == null ? null :
                    (Bitmap) bundle.getParcelable(DecodeMainThread.BARCODE_BITMAP);//���ñ����߳�
            ...
            LKLogUtil.e("Got return scan result message");
            activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
            activity.finish();

        } else if (message.what == R.id.launch_product_query) {
            LKLogUtil.e("Got product query message");

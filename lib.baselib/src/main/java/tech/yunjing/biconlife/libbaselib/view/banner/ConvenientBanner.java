package tech.yunjing.biconlife.libbaselib.view.banner;
...
import tech.yunjing.biconlife.libbaselib.view.banner.adapter.CBPageAdapter;
import tech.yunjing.biconlife.libbaselib.view.banner.holder.CBViewHolderCreator;
import tech.yunjing.biconlife.libbaselib.view.banner.listener.CBPageChangeListener;
import tech.yunjing.biconlife.libbaselib.view.banner.listener.OnItemClickListener;
import tech.yunjing.biconlife.libbaselib.view.banner.view.CBLoopViewPager;
...
public class ConvenientBanner<T> extends LinearLayout {
     ...
     * 单张轮播图片展示容器
     */
    private RelativeLayout rl_cb_showBackground;
    //    private boolean
    ...
    private void init(Context context) {
        ...
        rl_cb_showBackground = (RelativeLayout) hView.findViewById(R.id.rl_cb_showBackground);
        initViewPagerScroll();
    ...
    public ConvenientBanner setPages(CBViewHolderCreator holderCreator, List<T> datas) {
        ...
        if (mDatas.size() <= 1) {
            rl_cb_showBackground.setVisibility(VISIBLE);
            rl_cb_showBackground.bringToFront();
            stopTurning();
        } else {
            viewPager.setCanScroll(true);
            rl_cb_showBackground.setVisibility(GONE);
        }
...
//    }
//
//    public void setFanye(boolean fanye) {
//        isFanye = fanye;
//    }
    ...
    public ConvenientBanner setOnItemClickListener(OnItemClickListener onItemClickListener) {
        ...
        rl_cb_showBackground.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if(mDatas!=null && mDatas.size()>0){
                    viewPager.getOnItemClickListener().onItemClick(0);
                }
            }
        });
